    ...
    public partial class Cart : System.Web.UI.Page
    {
            ...
            if (e.CommandName == "updateCart")
            {
                ...
                for(int item=0; item < rCartItem.Items.Count; item++)
                {
                    if (rCartItem.Items[item].ItemType==ListItemType.Item || rCartItem.Items[item].ItemType == ListItemType.AlternatingItem)
                    {
                        ...
                        if (isTrue)
                        {
                            //updated cart item quantity in DB
        ...
        protected void rCartItem_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            if(e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                Label prodcutPrice = e.Item.FindControl("lblPrice") as Label;
                TextBox quantity = e.Item.FindControl("txtQuantity") as TextBox;
                decimal calTotalPrice = Convert.ToDecimal(prodcutPrice.Text) * Convert.ToDecimal(quantity.Text);
        ...
        //custom template class to add controls t te repeater header item 
